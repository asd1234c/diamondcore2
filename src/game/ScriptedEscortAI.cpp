...
void npc_escortAI::Start(bool bIsActiveAttacker, bool bRun, uint64 uiPlayerGUID, const Quest* pQuest, bool bInstantRespawn, bool bCanLoopPath)
{
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING))
    {
        error_log("DSCR: EscortAI attempt to Start while already escorting.");
        return;
    }
    ...
    debug_log("DSCR: EscortAI started with %u waypoints. ActiveAttacker = %d, Run = %d, PlayerGUID = %u", WaypointList.size(), m_bIsActiveAttacker, m_bIsRunning, m_uiPlayerGUID);
