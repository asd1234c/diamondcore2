...
void FollowerAI::EnterEvadeMode()
{
    ...
    {
        debug_log("DSCR: FollowerAI left combat, returning to CombatStartPosition.");
...
void FollowerAI::UpdateAI(const uint32 uiDiff)
{
    if (HasFollowState(STATE_FOLLOW_INPROGRESS) && !m_creature->getVictim())
    {
        if (m_uiUpdateFollowTimer <= uiDiff)
        {
            ...
            {
                debug_log("DSCR: FollowerAI is set completed, despawns.");
            ...
            if (Player* pPlayer = GetLeaderForFollower())
            {
                ...
                {
                    debug_log("DSCR: FollowerAI is returning to leader.");
            ...
            {
                debug_log("DSCR: FollowerAI failed because player/group was to far away or not found");
                m_creature->ForcedDespawn();
                return;
            }
...
void FollowerAI::StartFollow(Player* pLeader, uint32 uiFactionForFollower, const Quest* pQuest)
{
    if (m_creature->getVictim())
    {
        debug_log("DSCR: FollowerAI attempt to StartFollow while in combat.");
        return;
    }

    if (HasFollowState(STATE_FOLLOW_INPROGRESS))
    {
        error_log("DSCR: FollowerAI attempt to StartFollow while already following.");
        return;
    }
    ...
    if (m_creature->GetMotionMaster()->GetCurrentMovementGeneratorType() == WAYPOINT_MOTION_TYPE)
    {
        ...
        debug_log("DSCR: FollowerAI start with WAYPOINT_MOTION_TYPE, set to MoveIdle.");
    }
    ...
    debug_log("DSCR: FollowerAI start follow %s (GUID %u)", pLeader->GetName(), m_uiLeaderGUID);
}
...
Player* FollowerAI::GetLeaderForFollower()
{
        ...
        else
        {
            if (Group* pGroup = pLeader->GetGroup())
            {
                for (GroupReference* pRef = pGroup->GetFirstMember(); pRef != NULL; pRef = pRef->next())
                {
                    ...
                    {
                        debug_log("DSCR: FollowerAI GetLeader changed and returned new leader.");
    ...
    debug_log("DSCR: FollowerAI GetLeader can not find suitable leader.");
    return NULL;
}
