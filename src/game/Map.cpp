...
void Map::ProcesssPlayersVisibility()
{
    for (m_mapRefIter = m_mapRefManager.begin(); m_mapRefIter != m_mapRefManager.end(); ++m_mapRefIter)
    {
        ...
        CellPair cellpair(Diamond::ComputeCellPair(viewPoint->GetPositionX(), viewPoint->GetPositionY()));

        if (cellpair.x_coord >= TOTAL_NUMBER_OF_CELLS_PER_MAP || cellpair.y_coord >= TOTAL_NUMBER_OF_CELLS_PER_MAP)
            continue;

        Diamond::Player2PlayerNotifier notifier(*player);

        Cell cell(cellpair);
        cell.data.Part.reserved = ALL_DISTRICT;
        //cell.SetNoCreate();
        TypeContainerVisitor<Diamond::Player2PlayerNotifier, WorldTypeMapContainer > world_notifier(notifier);
...
void Map::ResetNotifies(uint16 notify_mask)
{
    ...
    Diamond::ResetNotifier reset(notify_mask);
    TypeContainerVisitor<Diamond::ResetNotifier, GridTypeMapContainer >  grid_notifier(reset);
    TypeContainerVisitor<Diamond::ResetNotifier, WorldTypeMapContainer > world_notifier(reset);
...
void Map::UpdatePlayerVisibility( Player* player, Cell cell, CellPair cellpair )
{
    ...
    Diamond::Player2PlayerNotifier pl_notifier(*player);
    TypeContainerVisitor<Diamond::Player2PlayerNotifier, WorldTypeMapContainer > player_notifier(pl_notifier);
...
{
    Diamond::VisibleNotifier notifier(*player, true);
