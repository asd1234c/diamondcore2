...
struct boss_deathwhisperAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (Phase == 2)
        {
            ...
            {
                 DoCast(m_creature->getVictim(), SPELL_INSIGNIFICANCE);
                Insignificance_Timer = 5000+rand()%40000;
            }
            ...
            {
                if (Unit* target = SelectUnit(SELECT_TARGET_RANDOM,0))
                     DoCast(target, m_bIsRegularMode ? SPELL_FROSTBOLT_VOLLEY : H_SPELL_FROSTBOLT_VOLLEY);
                FrostboltVolley_Timer = 40000;
            }
            ...
            if (Summon_Shade_Count < Summon_Cult_Count)
            {
                ...
                {
                     DoCast(m_creature, SPELL_VENGEFUL_SHADE);
                    Summon_Shade_Count++;
                    Summon_Shade_Timer = 25000;
                }
            ...
            {
                if (Unit* target = SelectUnit(SELECT_TARGET_RANDOM,0))
                     DoCast(target, m_bIsRegularMode ? SPELL_FROSTBOLT : H_SPELL_FROSTBOLT);
                Frostbolt_Timer = 15000;
            }
        ...
        {
            if (Unit* target = SelectUnit(SELECT_TARGET_RANDOM,0))
                 DoCast(target, m_bIsRegularMode ? SPELL_DEATH_AND_DECAY : H_SPELL_DEATH_AND_DECAY);
            DeathAndDecay_Timer = 30000;
        }
