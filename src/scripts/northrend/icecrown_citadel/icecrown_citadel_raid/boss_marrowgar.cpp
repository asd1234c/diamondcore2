...
struct boss_marrowgarAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        {
            DoCast(m_creature->getVictim(), m_bIsRegularMode ? SPELL_BONE_STORM : H_SPELL_BONE_STORM);
        ...
        if (BoneSpike_Timer < uiDiff)
        {
            if (Unit* target = SelectUnit(SELECT_TARGET_RANDOM,0))
                DoCast(target, m_bIsRegularMode ? SPELL_BONE_SPIKE : H_SPELL_BONE_SPIKE);
        ...
        {
            DoCast(m_creature->getVictim(), m_bIsRegularMode ? SPELL_COLDFLAME : H_SPELL_COLDFLAME);
            ColdFlame_Timer = 15000;
        }
        ...
        {
            DoCast(m_creature->getVictim(), m_bIsRegularMode ? SPELL_SABER_LASH : H_SPELL_SABER_LASH);
            SaberLash_Timer = 1000;
        }
