...
struct boss_saurfangAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (!m_creature->HasAura(BLOOD_LINK))
            DoCast(m_creature, BLOOD_LINK);

        if ((m_creature->GetHealth()*100 / m_creature->GetMaxHealth() < 30) && (!m_creature->HasAura(FRENZY)))
            DoCast(m_creature, FRENZY);

        /*
        if (m_creature->GetPower(m_creature->getPowerType())*100 / m_creature->GetMaxPower(m_creature->getPowerType()) == 100)
        {
        ...
        */
        ...
        {
            DoCast(m_creature->getVictim(), RUNE_OF_BLOOD);
            RuneOfBlood_Timer = 30000;
        }
