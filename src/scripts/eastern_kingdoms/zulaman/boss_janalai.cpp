...
//        DoZoneInCombat();
    ...
    bool HatchAllEggs(uint32 uiAction) //1: reset, 2: isHatching all
    {
...

        {
            ...
            Diamond::AllCreaturesOfEntryInRange check(m_creature, MOB_EGG, 100);
            Diamond::CreatureListSearcher<Diamond::AllCreaturesOfEntryInRange> searcher(m_creature, templist, check);

            TypeContainerVisitor<Diamond::CreatureListSearcher<Diamond::AllCreaturesOfEntryInRange>, GridTypeMapContainer> cSearcher(searcher);
    ...
    void Boom()
    {
...

        {
            ...
            Diamond::AllCreaturesOfEntryInRange check(m_creature, MOB_FIRE_BOMB, 100);
            Diamond::CreatureListSearcher<Diamond::AllCreaturesOfEntryInRange> searcher(m_creature, templist, check);
...
struct mob_amanishi_hatcherAI : public ScriptedAI
{
    ...
    bool HatchEggs(uint32 num)
    {
...

        {
            ...
            Diamond::AllCreaturesOfEntryInRange check(m_creature, 23817, 50);
            Diamond::CreatureListSearcher<Diamond::AllCreaturesOfEntryInRange> searcher(m_creature, templist, check);
