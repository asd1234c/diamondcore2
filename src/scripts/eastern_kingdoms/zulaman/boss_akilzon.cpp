...
struct boss_akilzonAI : public ScriptedAI
{
    ...
    void HandleStormSequence(Unit *Cloud) // 1: begin, 2-9: tick, 10: end
    {
        if (StormCount < 10 && StormCount > 1)
        {
            ...
            {
                Diamond::AnyAoETargetUnitInObjectRangeCheck u_check(m_creature, m_creature, 999);
                Diamond::UnitListSearcher<Diamond::AnyAoETargetUnitInObjectRangeCheck> searcher(m_creature, tempUnitMap, u_check);

                TypeContainerVisitor<Diamond::UnitListSearcher<Diamond::AnyAoETargetUnitInObjectRangeCheck>, WorldTypeMapContainer > world_unit_searcher(searcher);
                TypeContainerVisitor<Diamond::UnitListSearcher<Diamond::AnyAoETargetUnitInObjectRangeCheck>, GridTypeMapContainer >  grid_unit_searcher(searcher);
